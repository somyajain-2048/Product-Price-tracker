    ...
    >
      ...
      <div>
        <h2 className="font-display text-3xl xl:text-4xl font-light text-white leading-snug mb-3">
          The right price<br />
          <span className="italic text-violet-300">is worth waiting for.</span>
        </h2>
        <p className="text-white/50 text-sm font-light leading-relaxed mb-10">
          Welcome back. Track smarter, save more.
        </p>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[["50K+", "Products"], ["₹2M+", "Saved"], ["15K+", "Users"]].map(([val, label]) => (
          <div key={label} className="bg-white/8 border border-white/12 rounded-2xl p-4 text-center">
            <p className="font-display text-xl font-light text-white">{val}</p>
            <p className="text-[10px] text-white/40 mt-0.5 font-light tracking-wide uppercase">{label}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-50 px-5 py-12">
        <div className="w-full max-w-sm">
          ...
          {/* Heading */}
          <div className="mb-8">
            <h1 className="font-display text-3xl font-light text-gray-900 mb-1.5">Welcome back</h1>
            <p className="text-sm text-gray-400 font-light">Sign in to continue tracking your deals.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button type="button" className="text-xs text-indigo-500 hover:text-indigo-700 font-light transition-colors">
                  Forgot password?
                </button>
              </div>
